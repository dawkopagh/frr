...
void zebra_vxlan_stale_hrep_add(struct ipaddr vtep_ip, vni_t vni)
{
	struct zebra_evpn *zevpn = NULL;
	struct zebra_vtep *zvtep = NULL;
	struct in_addr vtep_ipv4 = vtep_ip.ipaddr_v4;
	...
	zvtep = zebra_evpn_vtep_find(zevpn, &vtep_ipv4);
	if (!zvtep) {
		 ...
		 */
		zvtep = zebra_evpn_vtep_add(zevpn, &vtep_ipv4, VXLAN_FLOOD_HEAD_END_REPL);
		if (!zvtep) {
